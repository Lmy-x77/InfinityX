...
function Library:RunElementEntrance(Holder: GuiObject, MatchColor: Color3, Delay: number)
    ...
    -- Cover ancorado embaixo: ao encolher, revela o conteúdo de cima para baixo,
    -- como se o elemento estivesse sendo "renderizado/construído" em vez de um
    -- fade plano. Não mexe em Position/Size do Holder real, então UIListLayout,
    -- UIGridLayout e AutomaticSize continuam intocados.
    local Cover = New("Frame", {
    ...
    -- Linha de brilho que acompanha a borda de revelação enquanto ela desce
    ...
    task.delay(Delay, function()
        ...
        local WipeTween = TweenService:Create(
            Cover,
        ...
        WipeTween.Completed:Once(function()
            if Cover and Cover.Parent then
                Cover:Destroy()
            end
        end)
...
function Library:FlushPendingEntrances(BaseDelay: number?)
    ...
    for Index, Entry in ipairs(Queue) do
        local Delay = BaseDelay + math.min((Index - 1) * 0.025, 0.5)
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        HeaderSeparator = Library:MakeLine(MainFrame, {
    ...
    local BuildEntranceChrome = {
        { Instance = TopBar, Property = "BackgroundTransparency" },
        { Instance = HeaderSeparator, Property = "BackgroundTransparency" },
        { Instance = DividerLine, Property = "BackgroundTransparency" },
        { Instance = Tabs, Property = "BackgroundTransparency" },
        { Instance = Container, Property = "BackgroundTransparency" },
        { Instance = BottomBackground, Property = "BackgroundTransparency" },
        { Instance = BottomBar, Property = "BackgroundTransparency" },
        { Instance = BottomSeparatorLine, Property = "BackgroundTransparency" },
    }
    if BackgroundImage then
        table.insert(BuildEntranceChrome, { Instance = BackgroundImage, Property = "ImageTransparency" })
    end

    local function HideBuildEntranceChrome()
        for _, Entry in BuildEntranceChrome do
            Entry.OriginalValue = Entry.Instance[Entry.Property]
            Entry.Instance.Visible = false
        end
    end

    local function RevealBuildEntranceChrome()
        local ChromeInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

        for _, Entry in BuildEntranceChrome do
            local Inst = Entry.Instance

            Inst[Entry.Property] = 1
            Inst.Visible = true

            local Tween = TweenService:Create(Inst, ChromeInfo, { [Entry.Property] = Entry.OriginalValue })
            table.insert(Library.ActiveEntranceTweens, Tween)
            Tween:Play()
        end
    end

    HideBuildEntranceChrome()
    ...
    local function PlayWindowEntrance()
        ...
        --// FASE 1: a Main Frame nasce sozinha, vazia, no centro da tela.
        -- Todo o conteúdo interno (sidebar, container, rodapé) já está escondido
        -- desde a criação da janela (HideBuildEntranceChrome), então aqui só existe
        -- literalmente o retângulo de fundo + outline da Main Frame.
        ...
        local ScaleEntranceInfo = TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out)

        local ScaleTween = TweenService:Create(MainFrameScale, ScaleEntranceInfo, { Scale = Library.DPIScale })
        ...
        --// FASE 2: a interface é "construída" sobre a Main Frame já pronta.
        -- Ordem: (1) cria TODOS os covers de tabs/groupboxes/elementos primeiro,
        -- já com o wipe agendado via delay — nada disso aparece ainda porque a
        -- sidebar/container continuam Visible = false; (2) só então revela o
        -- chrome, então tudo que aparece já nasce coberto e vai se "materializando"
        -- em cascata, respeitando a hierarquia (tabs → groupboxes → elementos).
        FadeTween.Completed:Once(function()
            ...
            task.delay(0.12, function()
                ...
                Library:FlushPendingEntrances(math.max(TabsDuration - 0.1, 0))

                RevealBuildEntranceChrome()
            end)
        end)
    end
    ...
    function Window:SetCornerRadius(Radius: number)
        ...
        DividerLine.Size = UDim2.new(0, 1, 1, -FooterHeight)

        Tabs.Size = UDim2.new(
            0,
            Window:GetSidebarWidth(),
            1,
            -(49 + FooterHeight) - InfoTabOffset
        )

        Container.Size = UDim2.new(
            1,
            -Window:GetSidebarWidth() - 1,
            1,
            -(49 + FooterHeight)
        )
        for _, Tab in Library.Tabs do
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                GroupboxArrow = New("ImageLabel", {
                    ...
                    Rotation = 180, -- aberto = seta para baixo
                    Position = UDim2.new(1, -12, 0, 17),
                    Size = UDim2.fromOffset(13, 13),
                    Visible = Info.Collapsible ~= false,
                    ZIndex = 2,
                    Parent = GroupboxHolder,
                })

                -- Botão invisível cobrindo o header inteiro (clicável)
            ...
            function Groupbox:Resize()
                local ContentHeight = (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49
                Groupbox.ExpandedHeight = ContentHeight

                if not Groupbox.Collapsed then
                    GroupboxHolder.Size = UDim2.new(1, 0, 0, ContentHeight)
                end
            end
            ...
            function Groupbox:SetCollapsed(Collapsed, SkipAnim)
                ...
                local ArrowGoal = Collapsed and 90 or 180
                local SizeGoal = Collapsed and UDim2.new(1, 0, 0, 35) or UDim2.new(1, 0, 0, Groupbox.ExpandedHeight)

                if SkipAnim then
                    GroupboxArrow.Rotation = ArrowGoal
                    GroupboxHolder.Size = SizeGoal
                    GroupboxContainer.Visible = not Collapsed
                    Groupbox.Tab:RefreshSides()
                    return
                end

                TweenService:Create(GroupboxArrow, TweenInfo.new(UI_CONFIG.AnimTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Rotation = ArrowGoal,
                }):Play()
                TweenService:Create(GroupboxHolder, TweenInfo.new(UI_CONFIG.AnimTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = SizeGoal,
                }):Play()
                ...
                task.delay(UI_CONFIG.AnimTime, function()
